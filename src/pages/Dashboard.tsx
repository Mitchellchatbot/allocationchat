...
const toUiConversation = (dbConv: DbConversation): Conversation & {
  ...
  visitor: {
    ...
    specialty: (dbConv.visitor as any)?.specialty || undefined,
    country_of_training: (dbConv.visitor as any)?.country_of_training || undefined,
    qualification_date: (dbConv.visitor as any)?.qualification_date || undefined,
  ...
  missedReply: (() => {
    ...
    const hasAgentMsg = msgs.some(m => m.sender_type === 'agent');
    return hasVisitorMsg && !hasAgentMsg && (dbConv.status === 'closed' || dbConv.status === 'pending');
  })(),
});
