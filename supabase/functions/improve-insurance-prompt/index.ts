...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `You are an expert prompt engineer specializing in conversational AI for doctor recruitment. Your job is to take a user's rough draft of personality or conversation instructions and rewrite them into a clear, structured prompt that a chat AI agent will follow during conversations with doctor candidates.

Rules for the improved prompt:
- Write in second person ("You must...", "Ask the doctor...")
- Use clear numbered steps with branching logic where appropriate
- Keep the tone directive but natural — the AI should still sound like a friendly human recruiter
- Preserve ALL the user's original intent and logic — don't remove anything they mentioned
- Add clarifying details where the draft is vague
- Include handling for edge cases the user may have missed
...
- Do NOT include any preamble or explanation. Output ONLY the improved prompt text.`;
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        system: systemPrompt,
        messages: [
          { role: 'user', content: `Here is my rough draft of personality/conversation instructions for our doctor recruitment chat AI agent:\n\n${draft}\n\nPlease rewrite this into a clear, structured prompt.` },
        ],
      }),
    });
